...
func get_all_animation_frames():
	...
	var player = find_child('Player') as Node3D
	var old_player_transform = player.global_transform
	var animation_player = find_child("AnimationPlayer") as AnimationPlayer
	var img_buffer = []
	var info = []
	var length = []
	for anim in animation_player.get_animation_list():
		length.append(animation_player.get_animation(anim).length)
	for anim in animation_player.get_animation_list():
	...
	for i in img_buffer:
		frames.append(len(i))
	return [img_array, animation_names,frames,state,length,fps]
...
func concatenate_images(buffer):
	...
	if buffer.size() == 1:
		...
		nb_of_rows = int(pow(len(buffer),0.5)) + 1
...
func _on_frames_per_second_text_changed(new_text):
	var fl = float(new_text)
	fl = clamp(fl,1,60)
	fps = fl
